...
Reescribe la línea `const CACHE_VERSION = '...';` con `<name>-<timestamp UTC>`
...
def main() -> int:
    ...
    new_text, n = re.subn(r"(const CACHE_VERSION = ')[^']*(';)",
                          rf"\g<1>{new_cache}\g<2>", text)
    if n != 1:
        print(f"build.py: esperaba 1 línea `const CACHE_VERSION = '...';` en sw.js, "
              f"encontré {n}.", file=sys.stderr)
        return 1

    SW.write_text(new_text, encoding="utf-8")
    print(f"sw.js → CACHE_VERSION = '{new_cache}'")
    return 0
